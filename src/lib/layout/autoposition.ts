 ...
 * face-bbox data. Output values are percentages (0..100) suitable for
 * CSS `object-position`.
 *
 * Rules:
...
export function autoPositionTransform(args: {
  ...
  const { photoWidth, photoHeight, faces, slot } = args;
  ...
  // Faces present → center the union bbox at slot center.
  if (faces.length > 0) {
